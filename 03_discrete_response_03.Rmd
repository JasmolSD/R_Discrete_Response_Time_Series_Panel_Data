...
```{r}
mod.logit.1 <-  glm(fracture ~ age + bmi, family=binomial(link = logit), data = df)
...
```{r}
round(exp(10*mod.logit.1$coefficients),3)
...
df$priorfracNo<-relevel(df$priorfrac, ref="No")
df$priorfracYes<-relevel(df$priorfrac, ref="Yes")
df$raterisk<-relevel(df$raterisk, ref="Less")
df$rateriskSame<-relevel(df$raterisk, ref="Same")
df$rateriskGreater<-relevel(df$raterisk, ref="Greater")

mod.logit.2 <- glm(formula = fracture ~ age + bmi + priorfrac + rateriskSame + rateriskGreater, family=binomial(link = logit), data = df)# uncomment and replace with your code
